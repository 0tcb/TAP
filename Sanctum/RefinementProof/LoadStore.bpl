procedure refinement_proof_step_load();
    ...
    requires (forall eid: enclave_id_t, va:vaddr_t :: enclave_metadata_is_initialized[eid] ==> (in_enclave_mem(va, enclave_metadata_ev_base[eid], enclave_metadata_ev_mask[eid]) <==> tap_enclave_metadata_addr_excl[enclave_id_bv2int(eid)][va]));
    requires (forall eid: enclave_id_t, va:vaddr_t :: tap_enclave_metadata_valid[enclave_id_bv2int(eid)] ==> (in_enclave_mem(va, enclave_metadata_ev_base[eid], enclave_metadata_ev_mask[eid]) <==> tap_enclave_metadata_addr_excl[enclave_id_bv2int(eid)][va]));
    ...
    requires (forall va: vaddr_t, eid: enclave_id_t, t_eid : tap_enclave_id_t, vpn : vpn_t :: (t_eid == enclave_id_bv2int(eid) && vpn == vaddr2vpn(va) && tap_enclave_metadata_valid[t_eid] && !tap_enclave_metadata_addr_excl[t_eid][va]) ==> tap_sanctum_perm_eq(tap_enclave_metadata_addr_valid[t_eid][va], ptbl_acl_map[cpu_ptbr, vpn]));
